...
impl eframe::App for PointViewer {
   ...
   fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {    
        ...
        ctx.input(|i| {
            if i.key_pressed(eframe::egui::Key::ArrowRight) && self.current_chunk < self.chunks.len() - 1 {
            ...
            egui::plot::Plot::new("waveforms")
                ...
                .show(ui, |plot_ui| {
                    ...
                    for (ch_num, x) in self.chunks[self.current_chunk].clone() {
                        plot_ui.line(
                            egui::plot::Line::new(x).color(color_same_as_egui((ch_num)as usize)).name(format!("ch #{}", ch_num + 1)));
                    }
                });
        });
   }
