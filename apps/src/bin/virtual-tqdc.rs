...
use tqdc::regs::{DeviceCtrl, Register16, Register32};
...
async fn handle_control_port(
    ...
    let sock = UdpSocket::bind(format!("{BOARD_IP}:{CONTROL_PORT}")).await?;
    ...
    loop {
        ...
        match msg {
            MlinkMessage::CtrlReq { header, regs } => {
                ...
                let msg = MlinkMessage::new_ctrl_acq(header.seq, header.dst, header.src, acq);
                sock.send_to(&MlinkMessage::to_datagram(&msg), to_addr)
                    .await?;
            }
...
async fn handle_stream_port(
    ...
    let rx = Arc::new(UdpSocket::bind(format!("{BOARD_IP}:{STREAM_PORT}")).await?);
    let tx = Arc::clone(&rx);
...
async fn main() -> io::Result<()> {
    ...
    let control_listener = tokio::spawn(handle_control_port(
        Arc::clone(&regs16),
        Arc::clone(&regs32),
        control_tx,
    ));

    let stream_listener = tokio::spawn(handle_stream_port(Arc::clone(&regs32), control_rx));
