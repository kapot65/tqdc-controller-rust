...
use processing::{frame_to_waveform, Algorithm, convert_to_kev, waveform_to_event, histogram::PointHistogram};
...
async fn main() {
    ...
    let central = crosses.iter().filter(|(_, waveforms)| {
        waveforms.len() <= 2  && waveforms.iter().any(|(ch, _)| *ch == 5)
    }).map(|(_, waveforms)| {
        waveforms.iter().filter(|(ch, _)| {
            *ch == 4 || *ch == 2 || *ch == 6 || *ch == 5
        }).map(|(_, amp)| *amp).sum::<f32>()
    }).collect::<Vec<_>>();


    let second = crosses.iter().filter(|(_, waveforms)| {
        waveforms.len() <= 2  && waveforms.iter().any(|(ch, _)| *ch == 1)
    }).map(|(_, waveforms)| {
        waveforms.iter().filter(|(ch, _)| {
            *ch == 0 || *ch == 3 || *ch == 5 || *ch == 1
        }).map(|(_, amp)| *amp).sum::<f32>()
