...
async fn main() {
    ...
    let double_crosses = crosses.iter()
    ...
    for (_, waveforms) in &double_crosses {
        ...
        let idx = if ch_1 < ch_2 {[ch_1, ch_2]} else {[ch_2, ch_1]};
    ...
    eframe::run_native("crosses", native_options, Box::new(|_| Box::new(CrossesViewer {
...
impl eframe::App for CrossesViewer {
   fn update(&mut self, ctx: &eframe::egui::Context, frame: &mut eframe::Frame) {    
        ...
        if ctx.input().key_pressed(eframe::egui::Key::ArrowRight) && self.current < self.filtered.len() - 1 {
            self.current += 1;
        }
        ...
        eframe::egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("apply").clicked() {
                    self.current = 0;
                    self.filtered = self.crosses.iter().filter(|(_, waveforms)| {
                        ...
                        (*time, waveforms.to_owned())
                    }).collect::<Vec<_>>();
                }
            });
            
            ui.horizontal(|ui| {
                ui.add(eframe::egui::Slider::new(&mut self.current, 0..=self.filtered.len() - 1)
                    .step_by(1.0));
