...
use analysis::{get_points_by_pattern, CorrectionCoeffs, Coeffs};
...
async fn main() {
    ...
    let coeffs = CorrectionCoeffs::load(&format!("/{db_root}/monitor.json"));
    ...
    let handles = points.iter().map(|filepath| {
        ...
        let (a, b) = {
            let Coeffs { a, b } = coeffs.get(fill, set).unwrap();
            (*a, *b)
        };
        ...
        tokio::spawn(async move {
            ...
            let secs = 
                point.channels.first().unwrap().blocks.first().unwrap().time / 
            ...
            count_rates.lock().await.insert(
                chrono::NaiveDateTime::from_timestamp_opt(secs as i64, 0).unwrap(), 
                (count_rate, count_rate as f32 / (a * (secs % 1_000_000) as f32 + b))
            );            
