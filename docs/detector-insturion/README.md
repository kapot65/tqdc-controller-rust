...
- Новый режим набора:
  ...
  - `frame-viewer (PILA)` - визуализация кадров в реальном времени для платы 3. В нынешнем варианте программа отображает кадры как они идут с платы, не корректируя базовую линию и не сдвигаясь на 2 бита.
...
5. Запустить `TQDC server (PILA)`. Проверить что порог и фильтр заданы правильно (первые 2 строки в терминале). Если все хорошо - можно начинать набор.
    ...
    *порог в `TQDC server (PILA)` должен быть в 4 раза меньше выставляемого в `AFI TQDC2 (PILA)`*
